...
public class NovedadesDocentesService {
    ...
    private final CursoCargoNovedadService cursoCargoNovedadService;
    private final FacultadService facultadService;
    private final GeograficaService geograficaService;

    public NovedadesDocentesService(Environment environment, CursoService cursoService, CursoCargoNovedadService cursoCargoNovedadService, FacultadService facultadService, GeograficaService geograficaService) {
        this.environment = environment;
        this.cursoService = cursoService;
        this.cursoCargoNovedadService = cursoCargoNovedadService;
    ...
    private void generateReport(String filename, Integer facultadId, Integer anho, Integer mes) {
        ...
        try {
            ...
            List<Integer> geograficaIds = cursoCargoNovedadService.findAllByFacultad(facultadId, anho, mes).stream().map(cursoCargoNovedad -> cursoCargoNovedad.getCurso().getGeograficaId()).toList();

            for (Geografica geografica : geograficaService.findAllByGeograficaIdIn(geograficaIds)) {
                ...
                var primera = true;
                for (CursoCargoNovedad novedad : cursoCargoNovedadService.findAllByFacultadAndGeograficaAndAlta(facultadId, geografica.getGeograficaId(), anho, mes)) {
                    if (primera) {
                        primera = false;
                ...
                primera = true;
                for (CursoCargoNovedad novedad : cursoCargoNovedadService.findAllByFacultadAndGeograficaAndCambio(facultadId, geografica.getGeograficaId(), anho, mes)) {
                    if (primera) {
                        primera = false;
                        detailTable = new PdfPTable(1);
                        detailTable.setWidthPercentage(70);
                ...
                primera = true;
                for (CursoCargoNovedad novedad : cursoCargoNovedadService.findAllByFacultadAndGeograficaAndBaja(facultadId, geografica.getGeograficaId(), anho, mes)) {
                    if (primera) {
                        primera = false;
                        detailTable = new PdfPTable(1);
                        detailTable.setWidthPercentage(70);
    ...
    private void addNovedadDetails(Document document, CursoCargoNovedad novedad) throws DocumentException {
...
//        document.add(Chunk.NEWLINE);
    ...
    static class HeaderFooter extends PdfPageEventHelper {
        ...
        public HeaderFooter(Integer anho, Integer mes, Facultad facultad) {
            this.anho = anho;
            this.mes = mes;
